...
#drop the CO, NO, NO2 and timeframe columns
data = data.drop(['CO','tf','NO2','NO','V0','V3'],axis=1)
...
data_norm = data_norm.drop(['NH3'], axis=1)

#standardize the NH3 concentrations by dividing all of them by 200 
data_norm_c = data.drop(['V1','V2','V4','V5'],axis=1)
data_norm_c = data_norm_c/200
...
def create_model(my_learning_rate, my_feature_layer):
    ...
    #set early stopping to enact if the loss to improve for 10 epochs
    ...
    #create one hidden layer, give it 16 nodes, and use a sigmoid activation layer
    model.add(tf.keras.layers.Dense(units=16, 
                                  activation='exponential', name='Hidden1'))
    
    model.add(tf.keras.layers.Dense(units=16, 
                                  activation='sigmoid', name='Hidden2'))
    
    #define the output layer with a sigmoid activation
        #it has three outputs, one for each gas concentration 
...
def train_model(model, dataset, epochs, label_name, early_stop, batch_size=None):
    ...
    label_1 = np.array(features.pop(label_name[0]))
    history = model.fit(x=features, y=[label_1],batch_size=batch_size,
...
# Train the model on the normalized training set. We're passing the entire normalized training set, but the model will only 
#use the features defined by the feature_layer.
epochs, mse, history = train_model(my_model, data_train, epochs_num, 
                          label_name, early_stop, #validation_split, 
                                   batch_size)

#print whether the model stopped training early and if so what epoch it stopped at
if len(history["mean_squared_error"]) == epochs_num:
    print('The model did not stop training early.')
if len(history["mean_squared_error"]) != epochs_num:
    print('The model stopped training at',len(history['loss']),'epochs becasue of early stopping.')
...
test_label_1 = np.array(test_features.pop(label_name[0]))
train_label_1 = np.array(train_features.pop(label_name[0]))

print("\n Evaluate the new model against the test set:")

my_test = my_model.evaluate(x = test_features, y = [test_label_1], batch_size=batch_size) 
